...
class Flow(object):
    def __init__(self, mesh, bndry, dt, theta, v_max, lambda_s, mu_s, rho_s, mu_f, rho_f, result,
        ...
        eW = MixedElement([eV, eU, eP, eR, eR])			# function space with multipliers
        #eW = MixedElement([eV, eU, eP])			# function space without multiplier
        ...
        self.writer.writerow(['time', 'mean pressure on outflow', 'pressure_jump', \
                              ...
                              'pressure difference', 'drag_lag1', 'drag_lag2', 'drag_eu1', 'drag_eu2',\
                              'lift_lag1', 'lift_lag2', 'lift_eu1', 'lift_eu2'])
    ...
    def save(self, t):
        ...
        def N(F, n):
            return 1/sqrt((inv(F.T)*n)[0]**2 + (inv(F.T)*n)[1]**2)*(inv(F.T)*n)
        ...
        # compute drag and lift without transforming to Eulerian quantities
        (drag_Lagrangian, lift_Lagrangian) = give_drag_and_lift(grad(self.v), self.n, self.p)

        # compute drag and lift after transformation to Eulerian quantities
        (drag_Eulerian, lift_Eulerian) = give_drag_and_lift(det(self.FF)*grad(self.v)\
                                * inv(self.FF.T), N(self.FF, self.n), det(self.FF)*self.p)

        S0 = integrate_over_interface((self.S_f*self.n)[0])
        S1 = integrate_over_interface((self.S_f*self.n)[1])
        R0 = integrate_over_interface(1/sqrt((inv(self.FF.T)*self.n)[0]**2 \
                + (inv(self.FF.T)*self.n)[1]**2)*(self.S_f*self.n)[0])
        R1 = integrate_over_interface(1/sqrt((inv(self.FF.T)*self.n)[0]**2 \
                + (inv(self.FF.T)*self.n)[1]**2)*(self.S_f*self.n)[1])
        pA = self.p((A.x(), A.y()))
        pB = self.p((B.x(), B.y()))
        p_diff = pB - pA
        ...
        #info("{}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}".format\
        #       (t, P, PI, Ax, Ay, p_diff, drag_Lagrangian, S0, drag_Eulerian, R0, \
        #       lift_Lagrangian, S1, lift_Eulerian, R1))
        self.writer.writerow([t, P, PI, Ax, Ay, p_diff, drag_Lagrangian, S0, drag_Eulerian, \
                R0, lift_Lagrangian, S1, lift_Eulerian, R1])

result = "results_FullALE"		# name of folder containing results
