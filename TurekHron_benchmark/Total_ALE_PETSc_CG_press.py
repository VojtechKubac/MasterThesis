...
import petsc_ts_solver_for_Total_ALE_CG as TS
...
class Flow(object):
    ...
    def __init__(self, mesh, bndry, interface, v_max, lambda_s, mu_s, rho_s, 
        ...
        eW = MixedElement([eU, eV, eP])			# final mixed element
        ...
        #info("Expression set.")
        bc_v_in     = DirichletBC(self.W.sub(1), self.v_in,            bndry, _INFLOW)
        bc_v_walls  = DirichletBC(self.W.sub(1), Constant((0.0, 0.0)), bndry, _WALLS)
        bc_v_circle = DirichletBC(self.W.sub(1), Constant((0.0, 0.0)), bndry, _CIRCLE)
        bc_u_in     = DirichletBC(self.W.sub(0), Constant((0.0, 0.0)), bndry, _INFLOW)
        bc_u_circle = DirichletBC(self.W.sub(0), Constant((0.0, 0.0)), bndry, _CIRCLE)
        #bc_u_walls  = DirichletBC(self.W.sub(0), Constant((0.0, 0.0)), bndry, _WALLS)
        bc_u_walls  = DirichletBC(self.W.sub(0).sub(1), Constant(0.0), bndry, _WALLS)
        bc_u_out    = DirichletBC(self.W.sub(0), Constant((0.0, 0.0)), bndry, _OUTFLOW)
        self.bcs = [bc_v_in, bc_v_walls, bc_v_circle, bc_u_in, bc_u_walls, bc_u_circle, bc_u_out]

        #info("Mesh BC.")
        bc_mesh = DirichletBC(self.W.sub(0), Constant((0.0, 0.0)), interface, _FSI)
        ...
        (u_, v_, p_) = TestFunctions(self.W)
        (self.u, self.v, self.p) = split(self.w)
        (self.udot, self.vdot, self.pdot) = split(self.wdot)
        ...
        udofs = W.sub(0).dofmap().dofs()
        vdofs = W.sub(1).dofmap().dofs()
        pdofs = W.sub(2).dofmap().dofs()
        ...
        vpdofs= [ *vdofs, *pdofs ]
        ISu= PETSc.IS().createGeneral( udofs )
        ISvp= PETSc.IS().createGeneral( vpdofs )
        ISvp.sort()

        pc = self.solver.pc
        pc.setType('fieldsplit')
        pc.setFieldSplitIS(('vp', ISvp), ('u', ISu))
